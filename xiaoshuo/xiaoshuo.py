...
def get_content(novel_name, href, headers):
    ...
    for link in tqdm(href):
        b = get_text(link, novel_name, headers)
        title_list.append(b[0])
        content_list.append(b[1])
        #
        # # print(b.result())
        # title = b.result()[0]
        # content = b.result()[1]
...
def save_text(novel_name, title_list, content_list):
    ...
    try:
        if upload.upload_file('F:/Git/xiaoshuo/novel/' + novel_name + '.txt'):
            print('上传成功')
            #删除文件
        ...
        if os.path.exists('F:/Git/xiaoshuo/novel/' + novel_name + '.txt'):  # 如果文件存在
            # 删除文件，可使用以下两种方法。
            os.remove('F:/Git/xiaoshuo/novel/' + novel_name + '.txt')
            # os.unlink(path)
            print('删除' + novel_name + '文件成功')
        else:
            print('文件不存在')
